...
const CustomerForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditing) {
      ...
      else { toast({ title: "Customer updated" }); navigate("/app/customers"); }
    } else {
      const { error } = await supabase.from("customers").insert({ ...payload, entity_id: selectedEntity.id, created_by: user.id });
      if (error) toast({ title: "Error", description: error.message, variant: "destructive" });
      else { toast({ title: "Customer created" }); navigate("/app/customers"); }
    }
    setSubmitting(false);
  };
  ...
  return (
    <div className="p-4 md:p-6">
      <Button variant="ghost" size="sm" className="mb-4" onClick={() => navigate("/app/customers")}>
        <ArrowLeft className="mr-1.5 h-3.5 w-3.5" /> Back
      </Button>
      <Card className="max-w-2xl">
        <CardHeader><CardTitle className="text-lg">{isEditing ? "Edit Customer" : "New Customer"}</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button type="button" variant="outline" onClick={() => navigate("/app/customers")}>Cancel</Button>
            </div>
          </form>
        </CardContent>
      </Card>
